...
class PaiementTaxeCourtierInit implements PaiementInit
{
    ...
    public function buildPaiement(?Facture $facture, ?DateTimeImmutable $dateOfPayment, ?Utilisateur $utilisateur, ?float $paidAmount): ?Paiement
    {
        ...
        $this->paiement->setType(PaiementCrudController::TAB_TYPE_PAIEMENT[PaiementCrudController::TYPE_PAIEMENT_SORTIE]);
        $this->paiement->setTypeFacture($facture->getType());
        $this->paiement->setDescription("Paiement de " . $this->serviceTaxes->getTaxe(false) . ". Facture n°" . $facture . ". Versement effectué le " . $this->serviceDates->getTexte($dateOfPayment));
